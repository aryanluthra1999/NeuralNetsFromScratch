...
def initialize_model(
        name, loss, layer_args, optimizer_args, logger=None, seed=None
):
...
class NeuralNetwork(ABC):
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        ### YOUR CODE HERE ###
        # Compute the loss.
        loss_grad = self.loss.backward(target, out)
        # Backpropagate through the network's layers.
        g = loss_grad
        backwards = list(self.layers)
        backwards.reverse()
            ...
            for param_name, param in layer.parameters.items():
                if param_name is not "null":
                    ...
                    if self.logger is not None:
                        ...
                        param_log["{}{}".format(param_name, i)]["max"] = np.max(
                            param
                        )
                        param_log["{}{}".format(param_name, i)]["min"] = np.min(
                            param
                        )
