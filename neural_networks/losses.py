...
class CrossEntropy(Loss):
    ...
    def forward(self, Y: np.ndarray, Y_hat: np.ndarray) -> float:
        ...
        ### YOUR CODE HERE ###
        m = Y.shape[0]
        assert Y.shape == Y_hat.shape, "Shapes of pred not same as true labels, True shape: " + str(
            Y.shape) + ". Pred shape: " + str(Y_hat.shape)
        return -1 / m * np.trace(Y @ np.log(Y_hat).T)
